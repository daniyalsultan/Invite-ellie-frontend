...
export function IntegrationsPage(): JSX.Element {
  ...
  useEffect(() => {
    ...
    if (connected === 'google' || connected === 'microsoft') {
      ...
      // Get stored workspace preference from localStorage
      const pendingWorkspaceId = localStorage.getItem(`pendingCalendarWorkspace_${platformKey}`);

      // Clear pending preference after use
      ...
      // If workspace preference was set, update the calendar after it's connected
      if (profile?.id && pendingWorkspaceId && email) {
        ...
        setTimeout(async () => {
          try {
            ...
            if (newCalendar) {
              // Update calendar with workspace preference
              ...
              if (recallaiUrl) {
                await fetch(recallaiUrl, {
                  ...
                  body: JSON.stringify({
                    default_workspace_id: pendingWorkspaceId,
                  }),
                });
                console.log(`[Integrations] Updated calendar ${newCalendar.id} with workspace preference: ${pendingWorkspaceId}`);
              }
            }
          } catch (error) {
            console.error('[Integrations] Error updating calendar workspace preference:', error);
  ...
  const openConnectModal = async (integration: CalendarIntegration) => {
    ...
    // Get user's workspace based on email
    try {
      const token = await ensureFreshAccessToken();
      if (!token) {
      ...
      console.error('Error loading workspace for calendar connection:', error);
      setConnectModalError('Failed to load workspace. Please try again.');
    }
  };
  ...
  const handleConnect = async () => {
    ...
    try {
      // Save workspace preference to localStorage for use after OAuth callback
      ...
      // Pass workspace_id to include in OAuth state
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-6">
          <div className="w-full max-w-sm rounded-[30px] bg-white p-6 text-center shadow-[0_25px_60px_rgba(0,0,0,0.15)]">
            ...
            <p className="mt-2 font-nunito text-sm text-[#5F6B7A]">
              Select a workspace to organize meetings from this calendar. This preference will be saved for all future meetings.
            </p>
            ...
            <div className="space-y-4 text-left">
              <div className="flex flex-col gap-2">
                <label className="font-nunito text-sm font-semibold text-[#25324B]">
                  Workspace: <span className="text-xs font-normal text-[#6B7A96]">(Optional but recommended)</span>
                </label>
                {connectModalWorkspaceId ? (
                  <div className="relative">
                    <select
                      value={connectModalWorkspaceId}
                      onChange={(event) => setConnectModalWorkspaceId(event.target.value || null)}
                      ...
                      disabled={connecting === pendingIntegration?.id}
                    >
                      <option value={connectModalWorkspaceId}>
                        {connectModalWorkspaceName || 'Workspace'}
                      </option>
                    </select>
